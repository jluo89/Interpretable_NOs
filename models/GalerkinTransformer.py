# try:
from .gt_libs.layers import *
from .gt_libs.utils_ft import *
# except:
...
class FourierTransformer2D(nn.Module):
    ...
    def forward(self, node, weight=None, boundary_value=None):
        '''
        - node: (batch_size, node_feats, n, n)
        ...
        pos, grid = self.get_pos_and_grid(node.size(0),node.size(-1))
        pos = pos.to(node.device)
        grid = grid.to(node.device)

        n_s = int(pos.size(1)**(0.5))

        node = node.view(bsz,n_s,n_s,-1) # permute the node from (batch size, nfeats, ns, ns) to (batch size, ns, ns, nfeats)
    ...
    def get_pos_and_grid(self, bsz, n_grid):
        # set grid and pos for input
        ...
        grid = torch.from_numpy(grid).requires_grad_(False).float().repeat([bsz, 1, 1, 1])
        pos = torch.from_numpy(pos).requires_grad_(False).float().repeat([bsz, 1, 1])
    ...
    def print_size(self):
        ...
        print(f'Total number of model parameters in GT: {nparams} (~{format_tensor_size(nbytes)})')
