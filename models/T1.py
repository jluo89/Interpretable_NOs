...
from .t1_layers import T1Layer1d
from .t1_layers import T1Layer2d
from .t1_utils import dct1d, idct1d, dct2d, idct2d
from .t1_utils import dfpnet_adaptive
...
class T1_2d(nn.Module):
    def __init__(
        ...
        self.fc0 = nn.Linear(self.in_channels, self.width)  # input channel is: (a(x), x)
        self.transform = transform # ["dct", "dft"]
    ...
    def forward(self, x, grid=None):
        ...
        if grid is not None and self.grid is True:
            x = torch.cat([x, grid], dim=1)
